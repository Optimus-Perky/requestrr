    ...
    public class ChatBot
    {
        ...
        private async Task DiscordModalSubmittedHandler(DiscordClient client, ModalSubmitEventArgs e)
        {
            try
            {
                ...
                else if (e.Values.FirstOrDefault().Key.ToLower().StartsWith("muvam"))
                {
                    await HandleMusicAlbumSearchModalAsync(e);
                }
            }
            catch (System.Exception ex)
            {
        ...
        private async Task HandleMusicAlbumSearchModalAsync(ModalSubmitEventArgs e)
        {
            KeyValuePair<string, string> firstTextbox = e.Values.FirstOrDefault();

            if (firstTextbox.Key.ToLower().StartsWith("muvam"))
            {
                // A modal submit is a brand new interaction with no response sent yet, unlike a
                // button click (which DiscordComponentInteractionCreatedHandler already
                // acknowledges before dispatch) - IMusicUserInterface's methods all call
                // EditOriginalResponseAsync, which needs a prior response to exist, so defer one
                // here first rather than duplicating every one of those methods for this path.
                ...
                await CreateMusicRequestWorkFlow(e, categoryId)
                    .HandleAlbumTermSearchAsync(firstTextbox.Value);
            }
        }
        ...
        private async Task HandleMusicRequestAsync(ComponentInteractionCreateEventArgs e)
        {
            ...
            else if (e.Id.ToLower().StartsWith("murbr"))
            {
                var categoryId = int.Parse(e.Id.Split("/").Skip(2).First());

                await CreateMusicRequestWorkFlow(e, categoryId)
                    .HandleBrowseAlbumsAsync(e.Id.Split("/").Last());
            }
            else if (e.Id.ToLower().StartsWith("muralsel"))
            {
                if (e.Values != null && e.Values.Any())
                {
                    var categoryId = int.Parse(e.Id.Split("/").Skip(2).First());
                    ...
                    await CreateMusicRequestWorkFlow(e, categoryId)
                        .HandleMusicAlbumSelectionAsync(e.Values.Single());
                }
            }
            else if (e.Id.ToLower().StartsWith("muralconf"))
            {
                var categoryId = int.Parse(e.Id.Split("/").Skip(2).First());
            ...
            else if (e.Id.ToLower().StartsWith("muralterm"))
            {
                var categoryId = int.Parse(e.Id.Split("/").Skip(2).First());
                var term = string.Join("/", e.Id.Split("/").Skip(3));

                await CreateMusicRequestWorkFlow(e, categoryId)
                    .HandleAlbumTermSearchAsync(term);
            }
            else if (e.Id.ToLower().StartsWith("murvasearch"))
            {
                var categoryId = int.Parse(e.Id.Split("/").Skip(2).First());

                await CreateMusicRequestWorkFlow(e, categoryId)
                    .ShowAlbumSearchModalAsync();
            }
        }
        ...
        private MusicRequestingWorkflow CreateMusicRequestWorkFlow(ModalSubmitEventArgs e, int categoryId)
        {
            return _musicWorkflowFactory.CreateRequestingWorkflow(e.Interaction, categoryId);
        }
